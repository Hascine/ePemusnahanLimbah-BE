...
const checkSigningAuthorization = async (authorizingUser, beritaAcara) => {
  ...
  try {
        ...
        try {
          ...
          if (beritaAcara.PermohonanPemusnahanLimbahs) {
            beritaAcara.PermohonanPemusnahanLimbahs.forEach(req => {
              ...
              if (req.is_produk_pangan === true && golonganName.includes('recall') && !golonganName.includes('prekursor')) {
          ...
          console.warn('[checkSigningAuthorization] Failed to determine golongan for signing dept matching:', gErr && gErr.message);
        }
    ...
    console.warn('[checkSigningAuthorization] External API authorization failed, falling back to database:', apiError.message);
...
try {
  ...
  const mapped = filteredRequests.map(reqItem => {
    ...
    const alasanPemusnahan = details && details.length > 0
      ? details.map(d => d.alasan_pemusnahan).filter(Boolean).join('; ')
      : '';
    ...
    if (reqItem.ApprovalHistories && reqItem.ApprovalHistories.length > 0) {
      ...
      const verifikasiApproval = reqItem.ApprovalHistories.find(ah => 
      ...
      verificationTimestamp = (reqItem.status === 'Completed' && reqItem.updated_at)
...
const createBeritaAcara = async (req, res) => {
  ...
  try {
    const { bagian, tanggal, waktu, lokasi_verifikasi, pelaksana_bagian, supervisor_bagian, pelaksana_hse, supervisor_hse, selectedRequestIds } = req.body;
    ...
    try {
      const externalRes = await axios.get(EXTERNAL_APPROVAL_URL);
      const items = Array.isArray(externalRes.data) ? externalRes.data : externalRes.data?.data || [];
      const beritaItems = items.filter(i => String(i.Appr_ApplicationCode || '') === 'ePengelolaan_Limbah_Berita_Acara');
      ...
      const isCreatorAllowed = creatorEntries.some(e => Number(e.Appr_No) === 1 && String((e.Appr_DeptID || '').toUpperCase()) === 'KL');
      if (!isCreatorAllowed) {
        await transaction.rollback();
        return res.status(403).json({ message: 'You are not authorized to create Berita Acara. Only HSE Supervisor/Officer may create.' });
      }
    } catch (err) {
      ...
      await transaction.rollback();
      return res.status(503).json({ message: 'Unable to verify creator authorization at this time. Please try again later.' });
    }

    // Basic server-side validation for required fields from the form
    if (!bagian || !tanggal || !waktu || !lokasi_verifikasi) {
      await transaction.rollback();
      return res.status(400).json({ message: 'Missing required fields: bagian, tanggal, waktu, or lokasi_verifikasi.' });
    }
    ...
    if (!waktuDate || isNaN(waktuDate.getTime())) {
      await transaction.rollback();
      return res.status(400).json({ message: 'Invalid waktu format. Expected ISO datetime (e.g. 2025-09-26T15:09:52).' });
    }
    ...
    try {
      const errDetails = {
        ...
        errors: Array.isArray(error.errors) ? error.errors.map(e => ({ message: e.message, path: e.path, value: e.value })) : undefined,
        stack: error.stack
      };
...
try {
  ...
  const transformed = events.map(ev => {
      ...
      if (e.SigningWorkflow && Array.isArray(e.SigningWorkflow.SigningWorkflowSteps)) {
          steps = e.SigningWorkflow.SigningWorkflowSteps.map(s => ({ step_id: s.step_id, step_level: s.step_level, status: 'pending' }));
      }
...
const getBeritaAcaraById = async (req, res) => {
  try {
    ...
    try {
      ...
      console.warn('[getBeritaAcaraById] External API failed, falling back to database signers:', externalError.message);
    ...
    const pendingSteps = workflowSteps.filter(s => s.status === 'pending' && s.step_level >= 2)
...
const signBeritaAcara = async (req, res) => {
  ...
  try {
    ...
    if (!isSigner) {
      ...
      console.warn('Unauthorized sign attempt for Berita Acara', { berita_acara_id: event.berita_acara_id, current_signing_step_id: event.current_signing_step_id, authorizingUser: authorizingUser && authorizingUser.log_NIK });
      await transaction.rollback();
    ...
    if (currentStepLevel === 3) {
      try {
        ...
        const userApprovals = appItems.filter(item => String(item.Appr_ID) === String(authorizingNik) && Number(item.Appr_No) === 3);
    ...
    if (currentStep && typeof currentStep.required_signatures !== 'undefined' && currentStep.required_signatures !== null) {
      ...
      const currentStepLevel = (currentStep && currentStep.step_level) ? Number(currentStep.step_level) : (event.current_signing_step_id ? Number(event.current_signing_step_id) : null);
      if (currentStepLevel === 2) {
        ...
        const hasRecall = Array.from(golonganNames).some(n => n.includes('recall'));
        const hasRecallPrecursor = Array.from(golonganNames).some(n => n.includes('recall') && n.includes('prekursor'));
        const hasOther = Array.from(golonganNames).some(n => !(n.includes('prekursor') || n.includes('oot') || n.includes('recall')));
    ...
    if (currentStep && currentStep.step_level === 3) {
      ...
      // For other levels, count total signatures for current step
    ...
    const refreshedEvent = await BeritaAcara.findByPk(event.berita_acara_id, {
      include: [{ model: PermohonanPemusnahanLimbah, include: [DetailLimbah, GolonganLimbah, JenisLimbahB3] }, { model: SigningWorkflowStep, include: [SigningWorkflowSigner] }]
    });

    const message = completedSignatures >= requiredSignatures 
      ? (refreshedEvent && refreshedEvent.current_signing_step_id ? 'Berita Acara signed successfully. Advanced to next step.' : 'Berita Acara signed successfully. All signatures complete.')
      : `Berita Acara signed successfully. ${requiredSignatures - completedSignatures} more signature(s) required for this step.`;
